...
class Experiment():
	...
	def resume(self, model, optim, weights_fpath=None, optim_path=None):  # 重新开始
	...
	def append_history_to_file(self, dset_type, loss):
		fpath = os.path.join(self.history_dir, dset_type + '.csv')
		with open(fpath, 'a') as f:
			f.write('{},{}\n'.format(self.epoch, loss))

	def save_history(self, dset_type, loss):
		self.loss_history[dset_type] = np.append(
			self.loss_history[dset_type], loss)
		self.append_history_to_file(dset_type, loss)
	...
	def save_weights(self, model, trn_loss, val_loss):
		weights_fname = self.name + '-weights-%d-%.3f-%.3f.pth' % (self.epoch, trn_loss, val_loss)
	...
	def load_weights(self, model, fpath):
		...
		print(state['val_loss'], state['val_err'])
